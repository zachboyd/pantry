...
public struct SafeViewModelFactory {
    ...
    /// Create PantryTabViewModel
    public func makePantryTabViewModel() throws -> PantryTabViewModel {
        let pantryItemService = try container.getPantryItemService()
        let householdService = try container.getHouseholdService()

        let dependencies = PantryTabDependencies(
            pantryItemService: pantryItemService,
            householdService: householdService
        )

        return PantryTabViewModel(dependencies: dependencies)
    }
