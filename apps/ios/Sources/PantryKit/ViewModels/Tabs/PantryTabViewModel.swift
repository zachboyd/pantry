...
// MARK: - PantryTabViewModel

/// ViewModel for the Pantry tab - manages pantry items and inventory
@Observable @MainActor
public final class PantryTabViewModel: BaseReactiveViewModel<PantryTabViewModel.State, PantryTabDependencies> {
    ...
    public struct State: Sendable {
        ...
        var sortOption: PantrySortOption = .name
    ...
    public enum PantrySortOption: String, CaseIterable, Sendable {
        case name
        case category
    ...
    public var sortOption: PantrySortOption {
    ...
    public var displayedItems: [PantryItem] {
    ...
    public required init(dependencies: PantryTabDependencies) {
        let initialState = State()
        super.init(dependencies: dependencies, initialState: initialState)
        Self.logger.info("🥫 PantryTabViewModel initialized")
    }

    public required init(dependencies: PantryTabDependencies, initialState: State) {
        super.init(dependencies: dependencies, initialState: initialState)
    }
    ...
    override public func refresh() async {
        Self.logger.debug("🔄 PantryTabViewModel refresh")
    ...
    public func loadPantryItems(for householdId: String) async {
        await executeTask(.load) {
            await self.performLoadPantryItems(for: householdId)
        }
    }

    /// Add a new pantry item
    public func addItem(_ item: PantryItem) async -> Bool {
        Self.logger.info("➕ Adding pantry item: \(item.name)")

        let result: Void? = await executeTask(.addItem) {
            try await self.dependencies.pantryItemService.addItem(item)
    ...
    public func updateItem(_ item: PantryItem) async -> Bool {
        Self.logger.info("✏️ Updating pantry item: \(item.name)")

        let result: Void? = await executeTask(.updateQuantity) {
            try await self.dependencies.pantryItemService.updateItem(item)
    ...
    public func removeItem(_ item: PantryItem) async -> Bool {
        Self.logger.info("🗑️ Removing pantry item: \(item.name)")

        let result: Void? = await executeTask(.removeItem) {
            try await self.dependencies.pantryItemService.deleteItem(id: item.id)
    ...
    private func performLoadPantryItems(for householdId: String) async {
        Self.logger.info("📡 Loading pantry items for household: \(householdId)")

        updateState { $0.viewState = .loading }

        do {
            let items = try await dependencies.pantryItemService.getItems(for: householdId)
...
public extension PantryTabViewModel {
    /// Get items expiring within a certain number of days
    func getItemsExpiring(within days: Int) -> [PantryItem] {
    ...
    func getItems(in category: ItemCategory) -> [PantryItem] {
